...
namespace Blanfordia\Visitors\Vendor\Laravel\Controllers;
...
use Blanfordia\Visitors\Vendor\Laravel\Facade as Visitors;
...
class Stats extends Controller
{
	...
	{
		Session::put('visitors.stats.days', $this->getValue('days', 1));

		Session::put('visitors.stats.page', $this->getValue('page', 'visits'));

		$this->minutes = new Minutes(60 * 24 * Session::get('visitors.stats.days'));

        $this->authentication = app()->make('visitors.authentication');
	}
	...
	public function index()
	{
        ...
        {
            return View::make('blanfordia/visitors::message')->with('message', 'Authentication required');
        }

        if ( ! $this->hasAdminProperty())
        {
            return View::make('blanfordia/visitors::message')->with('message', 'User model misses admin property');
        }

        if ( ! $this->isAdmin())
        {
            return View::make('blanfordia/visitors::message')->with('message', 'You are not Admin');
        }

		return $this->showPage(Session::get('visitors.stats.page'));
	}
	...
	public function visits()
	{
		...
		(
			'datatables_ajax_route' => route('visitors.stats.api.visits'),
			'datatables_columns' =>
				'
		...
		return View::make('blanfordia/visitors::index')
			->with('sessions', Visitors::sessions($this->minutes))
			->with('title', 'Visits')
			->with('username_column', Visitors::getConfig('authenticated_user_username_column'))
			->with('datatables_data', $datatables_data);
	}

	public function log($uuid)
	{
		return View::make('blanfordia/visitors::log')
				->with('log', Visitors::sessionLog($uuid))
				->with('uuid', $uuid)
				->with('title', 'Log');
	}
	...
	{
		return View::make('blanfordia/visitors::summary')
				->with('title', 'Page Views Summary');
	}

	public function apiPageviews()
	{
		return Visitors::pageViews($this->minutes)->toJson();
	}

	public function apiPageviewsByCountry()
	{
		return Visitors::pageViewsByCountry($this->minutes)->toJson();
	}

	public function apiLog($uuid)
	{
		$query = Visitors::sessionLog($uuid, false);
	...
	public function getValue($variable, $default = null)
	{
		...
		{
			$value = Session::get('visitors.stats.'.$variable, $default);
		}
	...
	{
		return View::make('blanfordia/visitors::users')
			->with('users', Visitors::users($this->minutes))
			->with('title', 'Users')
			->with('username_column', Visitors::getConfig('authenticated_user_username_column'));
	}
	...
	{
		return View::make('blanfordia/visitors::events')
			->with('events', Visitors::events($this->minutes))
			->with('title', 'Events');
	}

	public function errors()
	{
		return View::make('blanfordia/visitors::errors')
			->with('error_log', Visitors::errors($this->minutes))
			->with('title', 'Errors');
	}

	public function apiErrors()
	{
		$query = Visitors::errors($this->minutes, false);
	...
	{
		$query = Visitors::events($this->minutes, false);
	...
	{
		$username_column = Visitors::getConfig('authenticated_user_username_column');

		return Datatables::of(Visitors::users($this->minutes, false))
	...
	public function apiVisits()
	{
		...
		$query = Visitors::sessions($this->minutes, false);
		...
		return Datatables::of($query)
				...
				{
					$uri = route('visitors.stats.log', $row->uuid);
